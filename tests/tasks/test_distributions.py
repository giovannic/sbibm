...
def test_hierarchical_distribution_construction():
    ...
    # Local: depends on global, 4D Normal (1D per site, 4 sites)
    def local_dist_fn(global_params, n_local):
        batch_shape = global_params.shape[:-1]
        return pdist.Normal(0.0, 1.0).expand(list(batch_shape) + [n_local]).to_event(1)

    hier_dist = HierarchicalDistribution(
        global_dist, local_dist_fn, dim_global=2, dim_local=1, n_local=4
    )
...
def test_hierarchical_distribution_log_prob():
    ...
    hier_dist = HierarchicalDistribution(
        global_dist, local_dist_fn, dim_global=2, dim_local=1, n_local=3
    )
