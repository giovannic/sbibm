...
def test_prior_shape(n_l, dim):
    ...
    task = HierarchicalGaussianLinearUniform(n_l=n_l, dim_local_per_context=(dim - 1) // n_l)
...
def test_prior_structure():
    ...
    dim = 26
    prior_bound = 10.0
    task = HierarchicalGaussianLinearUniform(n_l=n_l, dim_local_per_context=(dim - 1) // n_l, prior_bound=prior_bound)
    prior = task.get_prior()
...
def test_reference_posterior_not_implemented():
    ...
    task = HierarchicalGaussianLinearUniform(n_l=3, dim_local_per_context=5)
