...
def test_prior_structure():
    """Test prior structure: global covariance params and local means."""
    n_l = 5
    task = HierarchicalSLCP(n_l=n_l)
    prior = task.get_prior()
    ...
    # First 2 params are s1, s2: should be Uniform(0.5, 3.0)
    s_params = samples[:, :2]
    assert s_params.min() >= 0.5
    assert s_params.max() <= 3.0

    # Third param is rho: should be Uniform(-3, 3)
    rho_param = samples[:, 2]
    assert rho_param.min() >= -3.0
    assert rho_param.max() <= 3.0

    # Remaining 2*n_l params are local means: should be Uniform(-3, 3)
    local_params = samples[:, 3:]
    assert local_params.shape[1] == 2 * n_l
    assert local_params.min() >= -3.0
    assert local_params.max() <= 3.0
