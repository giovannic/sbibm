...
def test_hierarchical_deepset_forward_pass():
    ...
    mask = torch.ones(batch_size, n_set)
    set_size = torch.full((batch_size,), float(n_set))

    # Forward pass
    log_prob_local, log_prob_global = model(x, y_local, y_global, set_size, mask)
...
def test_hierarchical_deepset_different_set_sizes(n_set_max):
    ...
    y_global = torch.randn(batch_size, dim_global)
    mask = torch.ones(batch_size, n_set_max)
    set_size = torch.full((batch_size,), float(n_set_max))
